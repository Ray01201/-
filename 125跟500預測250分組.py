...
def main():
    ...
    # 🌟【核心修改】依據 24 個特徵的組合進行分組
    # ngroup() 會將特徵完全相同的行分配同一個整數 ID (0, 1, 2...)
    ...
    print("=== Configuration Summary (Strict Grouping Control) ===")
    ...
    for model_name, model_obj in all_models.items():
        ...
        # 🌟 將自動算好的 24 特徵相同 groups 帶入驗證中
        for train_idx, test_idx in gkf.split(X, y, groups=groups):
            X_train, X_test = X.iloc[train_idx].copy(), X.iloc[test_idx].copy()
            y_train, y_test = y.iloc[train_idx], y.iloc[test_idx]

            # 💡 如果是 LGBM，目標變數轉換為等效電導 (y / 電壓)
            if is_lgbm:
    ...
    print("\n" + "="*20 + " 12 Models CV Performance Summary (Strict Multi-Feature Grouping) " + "="*20)
